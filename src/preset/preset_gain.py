...
class PresetGain:
    ...
    Self variables :    - gain_in: ndarray / if DB (Ndet, 2) / if UWB (Ndet)
                        ...
                        - all_gain: ndarray / if DB (1, Ndet, 2) / if UWB (1, Ndet)
    ...
    def _get_input_gain(self):
        ...
        This method sets the `gain_in`, `gain_iter`, and `all_gain`
        ...
        Attributes:
            ...
            all_gain (numpy.ndarray): An array containing the gain iteration values.
        ...
        if self.preset_qubic.params_qubic['instrument'] == 'UWB':
            gain_in = np.random.normal(1, self.preset_qubic.params_qubic['GAIN']['sig_gain'], self.preset_qubic.joint_in.qubic.ndets)
        else:
            gain_in = np.random.normal(1, self.preset_qubic.params_qubic['GAIN']['sig_gain'], (self.preset_qubic.joint_in.qubic.ndets, 2))

        self.gain_in = join_data(self.preset_tools.comm, gain_in)
        
        if self.preset_qubic.params_qubic['GAIN']['fit_gain']:
            gain_err = 0.2
            gain_iter = np.random.uniform(self.gain_in - gain_err/2, self.gain_in + gain_err/2, self.gain_in.shape)
        else:
            gain_iter = np.ones(self.gain_in.shape)
            
        self.gain_iter = join_data(self.preset_tools.comm, gain_iter)
        self.all_gain = np.array([self.gain_iter])